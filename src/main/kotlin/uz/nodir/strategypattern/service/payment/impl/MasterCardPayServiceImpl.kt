...
@Service
class MasterCardPayServiceImpl: PayService {
    ...
    override fun process(requestDTO: PaymentRequestDTO): PaymentResponseDTO {
        log.info("Received command to pay from MASTER CARD with request dto: $requestDTO")
        ...
        log.info("Finished command to pay from MASTER CARD with response dto: $response")
        return response
    }

    override fun type(): Processing {
       return Processing.MASTERCARD
    }
}